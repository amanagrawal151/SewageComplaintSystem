...
router.get('/status' ,async (req,res,next) => {
    ...
    data = await Complaint.find({email : curuser.email}) ;
    console.log(data)
    res.render('status' , {
        data ,
        curuser
...
router.get('/jeng', ensureAuthenticated,async (req,res,next) => {
    curuser = req.session.passport.user ;
    data = await ComplaintMapping.find({engineerName : curuser.username}) ;
    if(data)
    {
    data2 = await Complaint.find() ;
    var data3 = [];
    await data2.forEach(element => {
        data.forEach(element2 => {
            if(element2.complaintID == String(element._id))
            {
                data3.push(element) ;
            }
...
router.post('/delete' ,async (req,res,next) => { 
    console.log("here") ;
    _id2 = new ObjectId(req.body.complaintID) ;
    console.log(_id2) ;
    console.log(req.body.complaintID);
    await Complaint.deleteOne({_id : req.body.complaintID}) ;
    res.render('status') ;
})
...
router.post('/Resolved' ,async (req,res,next) => {
    const _id2 = new ObjectId(req.body.complaintID);
    await Complaint.updateOne({_id : _id2} , {status : "Resolved"}) ;
    res.redirect('/jeng') ;
})
